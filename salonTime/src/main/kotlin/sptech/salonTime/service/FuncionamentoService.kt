...
import sptech.salonTime.exception.AtributoInvalidoPatchException
import sptech.salonTime.exception.FuncionamentoNaoEncontradoException
...
class FuncionamentoService(private val repository: FuncionamentoRepository) {
    ...
    fun editar(id: Int, atributo: String, novoValor: String): Funcionamento {
        ...
        try {
            val funcionamentoAtualizado = when (atributo) {
                ...
                else -> throw AtributoInvalidoPatchException("Atributo inválido: $atributo")
            }
            return repository.save(funcionamentoAtualizado)
        }catch (e: Exception){
            throw AtributoInvalidoPatchException("Erro ao atualizar o atributo: $atributo. Verifique o valor fornecido.")
        }
    }
    ...
    fun editarTudo(id: Int, dados:Funcionamento): Funcionamento? {
        ...
        val funcionamentoExistente = repository.findById(id)
            .orElseThrow { FuncionamentoNaoEncontradoException("Funcionamento não encontrado") }
